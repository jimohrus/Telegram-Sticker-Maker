...
class WebMConverterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("WebM Video Sticker Converter for Telegram")
        ...
        # Title
        tk.Label(main_frame, text="GIF to WebM Sticker Converter", font=("Arial", 16, "bold"), bg="#2c2f33", fg="#ffffff").pack(pady=10)
        tk.Label(main_frame, text="Create Telegram-compliant video stickers", font=("Arial", 10), bg="#2c2f33", fg="#bbbbbb").pack(pady=5)
    ...
    def resize_frame(self, frame, size):
        """Resize a single frame to fit exactly within target size."""
    ...
    def create_webm(self, input_path, output_path, duration):
        """Convert GIF to VP9 WebM with FFmpeg, ensuring Telegram compliance."""
        try:
            # Adjust duration if necessary
            total_duration = min(duration, MAX_DURATION)
            setpts = f"setpts={total_duration/MAX_DURATION}*PTS" if total_duration > MAX_DURATION else "setpts=PTS"

            # FFmpeg command for VP9 WebM with transparency
            crf = 30  # Start with lower CRF for better quality
            max_attempts = 5
            bitrate = 150  # Start with a low bitrate (kbps) to control file size
            for attempt in range(max_attempts):
                cmd = [
                    ...
                    "-b:v", f"{bitrate}k",
                    "-crf", str(crf),
                    "-vf", f"{setpts},fps={TARGET_FPS},scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=black@0",
                    "-an",  # No audio
                    "-y",  # Overwrite output
                    output_path
                ]
                self.status_label.config(text=f"Converting with CRF={crf}, Bitrate={bitrate}k...")
                self.root.update()
                subprocess.run(cmd, check=True, capture_output=True)
                if os.path.getsize(output_path) / 1024 <= MAX_SIZE_KB:
                    break
                crf += 5  # Increase CRF to reduce quality and file size
                bitrate = int(bitrate * 0.8)  # Reduce bitrate by 20% each attempt
                if attempt == max_attempts - 1:
                    messagebox.showwarning("Warning", "WebM exceeds 256 KB. Using highest compression.")
    ...
    def convert(self):
        ...
        try:
            ...
            self.create_webm(os.path.join(temp_dir, "frame_%04d.png"), webm_path, total_duration)
